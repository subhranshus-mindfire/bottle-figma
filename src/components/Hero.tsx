...
const Hero = () => {
  ...
  const bottleTopY = useTransform(scrollYProgress, [0, isDesktop ? 0.185 : 0.12], [-50, isDesktop ? 1150 : 1200])
  const bottleBottomY = useTransform(scrollYProgress, [0, isDesktop ? 0.185 : 0.12], [-350, isDesktop ? 400 : 450])
    ...
    <>
      <div className="flex flex-col justify-center items-center my-6 gap-8 overflow-x-hidden" ref={ref}>
        <nav className='lg:h-[80px] flex justify-center lg:justify-between w-full lg:px-44'>
          ...
          <div className="right hidden md:block">
            {["Shop", "Contact Us", "About", "Journal", "Custom"].map((item: string) => {
              return (
                <button className="mx-8 text-[20px]">{item}</button>
              )
            })}
        ...
        <div className="absolute inset-0 top-40 z-50 -translate-y-36 lg:-translate-y-0">
          <motion.div className="bottle-top" initial={{ y: 200 }} animate={{ y: -50 }} transition={{ duration: 2.5, delay: 1 }} style={{ y: bottleTopY }}>
            <img src={BottleTop} alt="Bottle Top" className="m-auto scale-50 lg:scale-100" />
          </motion.div>
          <motion.div className="bottle-bottom" initial={{ y: -550 }} animate={{ y: -350 }} transition={{ duration: 2.5, delay: 1 }} style={{ y: bottleBottomY }}>
            <img src={BottleBottom} alt="Bottle Bottom" className="m-auto scale-50 lg:scale-100" />
          </motion.div>
        </div>
        <motion.div className="circle bg-gradient-to-b from-[#00D1FF] to-[#1A83FF] h-[600px] w-[600px] rounded-full p-2 absolute top-40" initial={{ scale: 0.6 }} animate={{ scale: 1 }} transition={{ delay: 1, duration: 2 }}>
          <motion.div className="bg-white rounded-full h-[582px]" initial={{ scale: 0.97 }} animate={{ scale: 1 }} transition={{ delay: 1, duration: 2 }}>
          </motion.div>
        </motion.div>
          ...
          <div className="bg-[#20292C]">
            <div className="flex flex-col justify-center items-center gap-4 lg:gap-14 h-screen pt-[450px] py-16 lg:py-0 lg:pt-0">
              {varieties.map((variety: string, index: number) => (
                <Variety text={variety} icon={varieties_icon[index]} />
              ))}
            </div>
          </div>
        </div>
